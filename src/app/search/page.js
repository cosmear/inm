            ...
            <main className="max-w-[1440px] mx-auto px-6 lg:px-12 py-12 flex-grow w-full">
                <div className="flex flex-col lg:flex-row gap-12">
                    <aside className="w-full lg:w-1/4">
                        <div className="sticky top-28 glass-card p-6 rounded-2xl space-y-8 shadow-sm border border-white/60">
                            <div>
                    ...
                    <div className="w-full lg:w-3/4">
                        ...
                        {loading ? <div className="h-64 flex items-center justify-center"><span className="material-symbols-outlined animate-spin text-primary text-4xl">slow_motion_video</span></div> : properties.length === 0 ? <div className="flex flex-col items-center justify-center py-32 text-center glass-card rounded-2xl opacity-60 border border-stone-dark/10"><span className="material-symbols-outlined text-4xl mb-4 text-stone-dark/40">search_off</span><h3 className="font-serif text-xl sm:text-2xl text-stone-dark/80">No se encontraron resultados</h3><p className="text-stone-dark/60 max-w-xs mx-auto mt-2 text-sm">Intenta ajustar los criterios de búsqueda para encontrar lo que buscas.</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-10">{properties.map(prop => <PropertyCard key={prop.id} prop={prop} />)}</div>}
                    </div>
                </div>
            </main>
        </div>
