...
export default function EditarPropiedad() {
    ...
    const [form, setForm] = useState({
        ...
        video_url: '',
        plan_url: '', // File object or local URL
        title: '',
        title: '',
        description: '',
        price: '',
            ...
            <main className="flex-grow max-w-5xl mx-auto px-6 py-12 w-full flex flex-col md:flex-row gap-8 items-start">
                ...
                <div className="flex-grow w-full">
                    <h1 className="text-2xl font-serif text-stone-dark mb-8 flex items-center gap-3">
                        <span className="material-symbols-outlined text-blue-500 bg-blue-50 p-2 rounded-xl">edit</span>
                        Editando Propiedad
                    </h1>
                    ...
                    <div className="bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-stone-dark/5 min-h-[400px] flex flex-col justify-between">
                        <div className="space-y-8 flex-grow">
                            ...
                            {currentStep === 1 && (
                                <div className="space-y-8 animate-fade-in">
                                    <h2 className="text-lg font-medium text-stone-dark mb-4">Revisá la operación y tipo</h2>
                                    ...
                                    <div className="flex flex-col md:flex-row gap-6">
                                        <div className="flex-1">
                                            ...
                                            <select
                                                ...
                                                className="w-full bg-white border border-stone-dark/20 text-stone-dark rounded-xl px-4 py-3 text-sm focus:border-primary outline-none transition-all appearance-none"
                                            >
                                        ...
                                        <div className="flex-1">
                                            <label className="text-xs font-medium text-stone-dark/60 block mb-2">Subtipo</label>
                                            <select
                                                name="subtipo"
                                                value={form.subtipo}
                                                onChange={handleChange}
                                                disabled={!form.type || propertyTypes[form.type]?.length === 0}
                                                className="w-full bg-white border border-stone-dark/20 disabled:bg-stone-100 text-stone-dark rounded-xl px-4 py-3 text-sm focus:border-primary outline-none transition-all appearance-none"
                                            >
                            ...
                            {currentStep === 2 && (
                                <div className="space-y-8 animate-fade-in">
                                    <div>
                                        <label className="text-xs font-medium text-stone-dark/60 block mb-2">Dirección exacta</label>
                                        <input type="text" name="location" value={form.location} onChange={handleChange} className="w-full bg-white border border-stone-dark/20 rounded-xl px-4 py-3 text-sm focus:border-primary outline-none" />
                                    </div>

                                    <div className="flex flex-col md:flex-row gap-6">
                                        <div className="flex-1">
                                            <label className="text-xs font-medium text-stone-dark/60 block mb-2">Provincia</label>
                                            <select name="provincia" value={form.provincia} onChange={handleChange} className="w-full bg-white border border-stone-dark/20 rounded-xl px-4 py-3 text-sm focus:border-primary outline-none appearance-none">
                            ...
                            {currentStep === 3 && (
                                <div className="space-y-10 animate-fade-in">
                                            ...
                                            {[{ id: 'a_estrenar', label: 'A estrenar', value: 'A estrenar' }, { id: 'anos_antiguedad', label: 'Años de antigüedad', value: 'Años de antigüedad' }, { id: 'en_construccion', label: 'En construcción', value: 'En construcción' }].map((option) => (
                                                <div key={option.id} className="flex items-center gap-4">
                                                    <label className="flex items-center gap-3 cursor-pointer text-sm text-stone-dark group">
                                                        <input type="radio" name="age" value={option.value} checked={form.age === option.value} onChange={handleChange} className="w-5 h-5 accent-[#F06C00]" />
                                                        {option.label}
                                                    </label>
                                                    {form.age === 'Años de antigüedad' && option.value === 'Años de antigüedad' && (
                                                        <input type="number" name="age_years" value={form.age_years} onChange={handleChange} className="w-24 bg-white border border-stone-dark/20 rounded-xl px-4 py-2 text-sm focus:border-primary outline-none" />
                                                    )}
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-y-8 gap-x-6 max-w-lg">
                                        {[
                                            { label: 'Ambientes', field: 'ambientes' },
                                            { label: 'Dormitorios', field: 'bedrooms' },
                                            { label: 'Baños', field: 'bathrooms' },
                                            { label: 'Toilettes', field: 'toilettes' },
                                            { label: 'Cocheras', field: 'cocheras' },
                                        ].map(item => (
                                            <div key={item.field}>
                                                <label className="text-xs font-medium text-stone-dark/60 block mb-3">{item.label}</label>
                                                <div className="flex items-center gap-3">
                                                    <button type="button" onClick={() => handleNumberChange(item.field, -1)} className="size-8 rounded flex items-center justify-center bg-stone-100 hover:bg-stone-200">-</button>
                                    ...
                                    <div className="pt-6 border-t border-stone-dark/10">
                                        ...
                                        <div className="flex flex-col md:flex-row gap-6 max-w-lg">
                                            <div className="flex-1">
                                                <label className="text-xs font-medium text-stone-dark/60 block mb-2">Cubierta</label>
                                                <div className="flex bg-white border border-stone-dark/20 rounded-xl overflow-hidden focus-within:border-primary transition-all">
                                                    ...
                                                    <input type="number" name="area_covered" value={form.area_covered} onChange={handleChange} className="w-full px-4 py-3 text-sm outline-none" />
                                                </div>
                                            </div>
                                            <div className="flex-1">
                                                <label className="text-xs font-medium text-stone-dark/60 block mb-2">Total</label>
                                                <div className="flex bg-white border border-stone-dark/20 rounded-xl overflow-hidden focus-within:border-primary transition-all">
                                                    <span className="bg-stone-50 px-4 py-3 text-stone-dark/60 text-sm border-r border-stone-dark/20">m2</span>
                                                    <input type="number" name="area" value={form.area} onChange={handleChange} className="w-full px-4 py-3 text-sm outline-none" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {currentStep === 4 && (
                                <div className="space-y-6 animate-fade-in">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            <label className="text-xs font-medium text-stone-dark/60 block mb-2">Imágenes (Agregar más)</label>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="video_url" value={form.video_url} onChange={handleChange} className="w-full bg-white border border-stone-dark/20 rounded-xl px-4 py-3 text-sm focus:border-primary outline-none" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="md:col-span-2">
                                            ...
                                            <input type="text" name="title" value={form.title} onChange={handleChange} className="w-full bg-white border border-stone-dark/20 rounded-xl px-4 py-3 text-sm focus:border-primary outline-none" required />
                                        </div>
                                        <div className="md:col-span-2 mb-4">
                                            <label className="text-xs font-medium text-stone-dark/60 block mb-2">Descripción *</label>
                                            <textarea name="description" value={form.description} onChange={handleChange} className="w-full min-h-[140px] bg-white border border-stone-dark/20 rounded-xl px-4 py-3 text-sm focus:border-primary outline-none resize-y" required></textarea>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex bg-white border border-stone-dark/20 rounded-xl overflow-hidden focus-within:border-primary transition-all">
                                                ...
                                                <input type="number" name="price" value={form.price} onChange={handleChange} className="w-full px-4 py-3 text-sm outline-none" required />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="text-xs font-medium text-stone-dark/60 block mb-2">Expensas</label>
                                            <div className="flex bg-white border border-stone-dark/20 rounded-xl overflow-hidden focus-within:border-primary transition-all">
                                                <span className="bg-stone-50 px-4 py-3 text-stone-dark/60 text-sm border-r border-stone-dark/20">$</span>
                                                <input type="number" name="expenses" value={form.expenses} onChange={handleChange} className="w-full px-4 py-3 text-sm outline-none" />
                                            </div>
                                        </div>
                                        <div className="md:col-span-2 mt-2 border-b border-stone-dark/10 pb-6 mb-2">
                                            <label className="flex items-center gap-3 cursor-pointer">
                                                <input type="checkbox" name="credit_apt" checked={form.credit_apt} onChange={(e) => setForm({ ...form, credit_apt: e.target.checked })} className="w-5 h-5 accent-[#F06C00]" />
                                                <span className="text-sm font-medium text-stone-dark">Apto crédito</span>
                                            </label>
                                        </div>

                                        <div className="md:col-span-2 pt-2">
                                            <label className="text-xs font-medium text-stone-dark/60 block mb-2">Amenities</label>
                                            <input type="text" name="amenities" value={form.amenities} onChange={handleChange} className="w-full bg-white border border-stone-dark/20 rounded-xl px-4 py-3 text-sm focus:border-primary outline-none" />
                                        </div>
                                        ...
                                        <div className="md:col-span-2 pt-6 border-t border-stone-dark/10">
                                            ...
                                            <div className="w-full md:w-1/2">
                                                ...
                                                <select
                                                    ...
                                                    value={form.status}
                                                    onChange={handleChange}
                                                    className="w-full bg-white border border-stone-dark/20 text-stone-dark rounded-xl px-4 py-3 text-sm focus:border-primary outline-none appearance-none"
                                                >
                        ...
                        <div className="mt-10 pt-6 border-t border-stone-dark/10 flex items-center justify-between">
                            {currentStep > 1 ? (
                                <button type="button" onClick={handleBack} className="text-sm font-medium text-stone-dark/70 hover:text-primary flex items-center gap-1">
                                    <span className="material-symbols-outlined text-lg">chevron_left</span> Atrás
                                </button>
                            ) : <div></div>}

                            <div className="flex items-center gap-4">
                                <Link href="/admin/dashboard" className="px-6 py-2.5 rounded-xl border border-stone-dark/20 text-stone-dark text-sm font-medium hover:bg-stone-50">
                                    Cancelar
                                </Link>

                                {currentStep < 4 ? (
                                    <button type="button" onClick={handleNext} disabled={currentStep === 1 && !form.type} className="px-8 py-2.5 rounded-xl bg-[#F06C00] hover:bg-[#D96100] text-white text-sm font-medium shadow-sm">
                                        Continuar
                                    </button>
                                ) : (
                                    <button type="button" onClick={handleSubmit} disabled={loading || !form.title || !form.price || !form.description || form.images.length === 0} className="px-8 py-2.5 rounded-xl bg-[#F06C00] hover:bg-[#D96100] text-white text-sm font-medium shadow-sm flex items-center gap-2">
